...
Abstract class AbstractCommandHandel
{
    ...
    protected function setClusterExecClientByKey($key)
    {
        if ($this->redis instanceof RedisCluster) {
             ...
             */
            $redis = $this->redis;
            $client = $redis->getClientBySlotId(self::getRedisSlot($key));
            $redis->setDefaultClient($client);
        }
    }

    protected function getClusterExecClientByKey($key)
    {
        /**
         * @var $redis RedisCluster
         */
    ...
    function exec(...$data)
    {
        ...
        if ($this->redis instanceof RedisCluster) {
            $client = $this->getClusterExecClientByKey($this->key);
            if (!$this->redis->sendCommandByClient($command,$client)) {
                return false;
            }
            $recv = $this->redis->recvByClient($client);
            if ($recv === null) {
                return false;
            }
            return $this->getData($recv);

        } else {
            if (!$this->redis->sendCommand($command)) {
                return false;
            }
            $recv = $this->redis->recv();
            if ($recv === null) {
                return false;
            }
            return $this->getData($recv);
     ...
     * Time: 13:58
     */
    static function getRedisSlot(string $ptr): int
    {
        ...
        $crc = 0x0000;
        for ($i = 0; $i < strlen($ptr); $i++) $crc = $crcTable[(($crc >> 8) ^ ord($ptr[$i]))] ^ (($crc << 8) & 0x00FFFF);
        return $crc % 16384;
    }
